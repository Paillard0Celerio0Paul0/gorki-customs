...
async function main() {
  ...
  // Créer des teams d'abord
  const blueTeam1 = await prisma.team.create({
    data: {
      name: 'Team Blue',
      side: 'BLUE',
      players: {
  ...
  const redTeam1 = await prisma.team.create({
    data: {
      name: 'Team Red',
      side: 'RED',
      players: {
  ...
  const game1 = await prisma.game.create({
    data: {
      ...
      redScore: 18,
      duration: 42,
      blueTeamId: blueTeam1.id,
      redTeamId: redTeam1.id,
    },
  })

  // Créer les teams pour la deuxième game
  const blueTeam2 = await prisma.team.create({
    data: {
  ...
  const redTeam2 = await prisma.team.create({
    data: {
  ...
  const game2 = await prisma.game.create({
    data: {
      ...
      redScore: 28,
      duration: 35,
      blueTeamId: blueTeam2.id,
      redTeamId: redTeam2.id,
    },
  })
